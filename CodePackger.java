...
class SourceCodeFile{
	...
	public SourceCodeFile(String firstLine,BufferedReader in) {
		dirname=chapter;
		filename=firstLine.
				substring(startMarker.length()).trim();
	 ...
	 public void writePacked(DataOutputStream out) {
		 try {
			 out.writeBytes(packMarker+dirname+"#"
					 +filename+eol);
...
public class CodePackger {
	private static final String usageString=
			...
			"jqava CodePackager packedFileName newDir\n"+
		...
		try {
			while((currentLine=in.readLine())!=null) {
				if(currentLine.startsWith(SourceCodeFile.startMarker)) {
					...
					Pr.error("file has no strat marker");
			}
		}catch(IOException e) {
